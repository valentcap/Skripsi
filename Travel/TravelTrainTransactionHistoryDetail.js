...
import { isIphoneXorAbove, popScreen, pushScreen, resetScreen } from '../../libs/navigation';
...
export default class extends Component {
    ...
    reOrder = (isReorder=false, SalesOrder=undefined) => {
        ...
        if(onAction) {
            if(isReorder){
                ...
                let { Booking } = SalesOrder
                let { originStation, destinationStation, TotalAdultPassenger } = Booking[0]
                let Origin1 = {
    ...
    renderSO = (SO, i) => {
        let { SalesOrderHeaderId, SalesOrderDetail, SalesOrderNo, Booking, TravelType, ShippingStatus, PaymentStatus } = SO
        let { Passenger, originStation, destinationStation, DepartureDate, ArrivalDate, TrainName,
            TotalAdultPassenger, PriceAdult, SubTotalBrutoAdult } = Booking[0]
        let { OrderStatus, SalesOrder } = this.state
            ...
            <View style={[styles.card]} key={i}>
                ...
                <View style={styles.cardContentContainer}>
                    ...
                    <View style={[styles.row, { margin: 20 }]}>
                        <AutoHeightImage width={50} source={require('../../assets/travel/Logo/KAI.png')} style={{ marginRight: 50 }}></AutoHeightImage>
                        <View style={styles.row}>
                            <AutoHeightImage width={15} source={require('../../assets/travel/Train-grey.png')} style={{ tintColor: black, marginRight: 10 }}></AutoHeightImage>
                    ...
                    <View style={[styles.passengercard, styles.item]}>
                        ...
                        {
                            Passenger.map((el, j) => {
                                let { Sallutation, Name, wagonName, wagonNumber, SeatRow, SeatPotition, Identity, Maturity } = el
                    ...
                    <Text style={[styles.textSmall, styles.sectionText, styles.item]}>Pesanan</Text>
        
                    <View style={{ flexDirection: 'row' }}>
                        <View style={{ flex: 2, justifyContent: 'center', alignSelf: 'flex-start' }}>
                            <AutoHeightImage
                                width={50}
                        ...
                        <View style={{ flex: 8 }}>
                            <Text style={[styles.black, {marginBottom: 5}]}>Tiket {TravelType} {i == 0 ? 'Pergi' : 'Pulang'} {i == 0 ? originStation.City : destinationStation.City} - {i == 0 ? destinationStation.City : originStation.City} (Dewasa)</Text>
                            <SpaceBetween>
                                <View style={styles.row}>
                                    <Text style={[styles.black, { marginRight: 10 }]}>{TotalAdultPassenger} x</Text>
                                    <Text style={styles.black}>{Rupiah(PriceAdult)}</Text>
                                </View>
                                <Text style={[styles.text, styles.bold, styles.black]}>{Rupiah(TotalAdultPassenger*PriceAdult)}</Text>
                            </SpaceBetween>
        
                            <Dash style={[styles.item, styles.separator]} dashGap={9} dashLength={7} dashColor={'darkgrey'} dashThickness={1} />

                            {/* <SpaceBetween>
                                <Text style={[styles.text, styles.black]}>Total Harga Tiket</Text>
                                <Text style={[styles.text, styles.bold, styles.black]}>{Rupiah(TotalAdultPassenger*PriceAdult)}</Text>
                            </SpaceBetween>

                            <SpaceBetween>
                                <Text style={[styles.text, styles.black]}>Biaya Layanan</Text>
                                <Text style={[styles.text, styles.bold, styles.black]}>{Rupiah(SalesOrderDetail[0].PPN)}</Text>
                            </SpaceBetween>
        
                            <Dash style={[styles.item, styles.separator]} dashGap={9} dashLength={7} dashColor={'darkgrey'} dashThickness={1} /> */}
        
                            <SpaceBetween>
                                <Text style={[styles.text, styles.black]}>Subtotal</Text>
                                <Text style={[styles.text, styles.bold, styles.black]}>{Rupiah(SubTotalBrutoAdult)}</Text>
                            </SpaceBetween>
                        </View>
                    </View>

                    {
                        PaymentStatus == STATUS.SUCCESS.CODE && ShippingStatus == STATUS.SUCCESS.CODE ? (
                            <Button
                                text={'Lihat Tiket Saya'}
                                style={{ borderColor: blue, borderWidth: 1 }}
                                onPress={() => this.reOrder(false, SalesOrderHeaderId) }
                                backgroundColor={'white'}
                                color={blue} />
                        ) : null
                    }
                    {
                        OrderStatus && (i == SalesOrder.length-1) ? (
                            <Button
                                text={'Status Pesanan'}
                                style={{ borderColor: blue, borderWidth: 1 }}
                                onPress={() => pushScreen(this.props.componentId, 'TravelOrderStatus', {
                                    SalesID: SalesOrderHeaderId
                                }) }
                                backgroundColor={'white'}
                                color={blue} />
                        ) : null
    ...
    render() {
        let { BookingTotal, PaymentStatus, ShippingStatus, SalesOrderDate, SalesOrder } = this.state
        let { CouponMappingCode, PaymentType, Payment, IsButtonCopyShow } = SalesOrder[0]
        let { isTimeout } = this.state
                ...
                <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
                    <View>
                        ...
                        <View style={styles.item}>
                            ...
                            <View>
                                {
                                    SalesOrder.map((SO, i) => {
                                        return (
                                            <SpaceBetween key={i}>
                                                <Text style={[styles.text, styles.black]}>Subtotal {i == 0 ? 'Pergi' : 'Pulang'}</Text>
                                                <Text style={[styles.text, styles.black]}>{Rupiah(SO.SalesOrderDetail[0].Price)}</Text>
                                            </SpaceBetween>
                                        )
                                    })
                                }
                                {
                                    SalesOrder[0].SalesOrderDetail[0].Discount > 0 ? (
                                        <SpaceBetween>
                                            ...
                                            <Text style={[styles.text, styles.black]}>{Rupiah(SalesOrder[0].SalesOrderDetail[0].Discount + (SalesOrder[1] ? SalesOrder[1].SalesOrderDetail[0].Discount : 0))}</Text>
                                        </SpaceBetween>
                                    ) : null
                                }
                                <SpaceBetween>
                            ...
                            {
                                (PaymentType.PartialPaymentCode && PaymentStatus == STATUS.FAILED.CODE) || (PaymentStatus == STATUS.SUCCESS.CODE && ShippingStatus == STATUS.FAILED.CODE) ? (
                                    <View style={[styles.infoBox, { marginTop: 8 }]}>
                                        <View style={[styles.row, { flex: 1 }]}>
                                            <Image source={require('../../assets/warning.png')} style={{ height: 20, resizeMode: 'contain', marginRight: 8 }}/>
                                            <Text style={[styles.text, styles.textSmall, styles.black, { flexShrink: 1 }]}>Biaya yang sudah dibayarkan akan dikembalikan ke Saldo Klik atau Limit Kartu Kredit / Akulaku kamu</Text>
                                        </View>
                                    </View>
                                ) : null
                            }
                    ...
                    <View style={[styles.popUpDetail]}>
                        ...
                        <View>
                            ...
                            <View style={styles.item}>
                                ...
                                {
                                    PaymentStatus != STATUS.SUCCESS.CODE || isTimeout ? (
                                        <Text style={[styles.text, {color: orange}]}>Batas waktu pembayaran habis</Text>
                                    ) : null
                                }
                                {
                                    PaymentStatus == STATUS.WAITING.CODE && !isTimeout ?
                                        (<Text style={[styles.text, styles.textLarge, { color: orange }]}>Batas Waktu : <TravelCountdown type={'text'} hours={Payment.ExpiredInHours} minutes={Payment.ExpiredInMinutes} seconds={Payment.ExpiredInSeconds} onTimeout={() => this.setState({ isTimeout: true })}/></Text>) : null
                                }
                            </View>
                        </View>
                        <Button
