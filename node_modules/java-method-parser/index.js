...
const parseClassName = file => {
	...
	//gabisa nangkep parent / interface
	// const classNameRegex = /public (?:final |static | )*class (.*)\s\{/;
	// const classNameRegex = /public(?:final |static | )*class (.*)(?:extends |implement | )(.*)\s\{/;
	const classNameRegex = /(.*)class\s+(\w+)(\s+extends\s+(\w+))?(\s+implements\s+([\w,\s]+))?\s*\{/;
	const arr = classNameRegex.exec(file);
	// console.log(arr);
	const [,, name,, parent,, itf] = classNameRegex.exec(file);
	// console.log(name, parent, itf);
	const obj = {
		className: name,
		classParent: parent,
		classInterface: itf
...
const parseInterfaces = rawInterfaces => {
	const interfaceRegex = /(\w+(,\w+)*)/g;
	let match;
	let interfaces = [];
	while ((match = interfaceRegex.exec(rawInterfaces))){
		// console.log(match);
		let [, result] = match;
...
const parse = file => {
	let obj = parseClassName(file);
	return {
		// package: parsePackageName(file),
		name: obj.className,
		parent: obj.classParent,
		interfaces: parseInterfaces(obj.classInterface),
